...
layout(push_constant) uniform PushConstants { vec3 camPos; }
pushConstants;
...
vec3  FresnelSchlickRoughness(float cosTheta, vec3 F0, float roughness) { return F0 + (max(vec3(1.0 - roughness), F0) - F0) * pow(max(1.0 - cosTheta, 0.0), 5.0); }
