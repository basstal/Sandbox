...
template <typename T> class CLASS() List : public std::vector<T>, public rfk::Object
{
  public:
    METHOD()
...
template <typename T> YAML::Node Sandbox::List<T>::SerializeToYaml()
{
...
template <typename T> bool Sandbox::List<T>::DeserializeFromYaml(const YAML::Node& node)
{
    const rfk::Class& objectType = T::staticGetArchetype();
...
template <typename T> rfk::Struct const& List<T>::getArchetype() const noexcept { return T::staticGetArchetype(); }
} // namespace Sandbox NAMESPACE()
