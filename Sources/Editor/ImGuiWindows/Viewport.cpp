...
{
	if (m_referenceGameObject == nullptr || m_imguiWindow == nullptr)
	{
		return;
	}
	...
	ImGuizmo::SetRect(ToFloat(rectPosition.x + m_startPosition.x), ToFloat(rectPosition.y + m_startPosition.y), ToFloat(extent2D.width), ToFloat(extent2D.height));
...
{
	// 计算目标分辨率的宽高比
	float targetAspectRatio = static_cast<float>(aspectWidth) / aspectHeight;

	// 计算实际分辨率的宽高比
	float actualAspectRatio = static_cast<float>(viewportWidth) / viewportHeight;
	...
	if (actualAspectRatio > targetAspectRatio)
	{
		...
		float scaleFactor = static_cast<float>(viewportHeight) / adjustedHeight;
		adjustedHeight = viewportHeight;
		adjustedWidth = static_cast<int>(adjustedWidth * scaleFactor);
		// 重新计算起始X坐标
		startX = (viewportWidth - adjustedWidth) / 2;
	}
	else if (actualAspectRatio < targetAspectRatio)
	{
		...
		float scaleFactor = static_cast<float>(viewportWidth) / adjustedWidth;
        adjustedWidth = viewportWidth;
        adjustedHeight = static_cast<int>(adjustedHeight * scaleFactor);
        // 重新计算起始Y坐标
        startY = (viewportHeight - adjustedHeight) / 2;
    }
