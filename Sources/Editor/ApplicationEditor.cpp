...
ApplicationEditor::ApplicationEditor()
{
    ...
    auto subpass = std::make_shared<Subpass>(m_device);
    subpass->BeginSubpassAttachments(VK_SAMPLE_COUNT_1_BIT);
    ...
    init_info.ImageCount = static_cast<uint32_t>(application->swapchain->swapchainVkImages.size());
...
void ApplicationEditor::CreateFramebuffer()
{
    ...
    // Create The Image Views
    {
        ...
        {
            info.image = application->swapchain->swapchainVkImages[i];
    ...
    // Create Framebuffer
    {
        ...
        info.width = application->swapchain->swapchainVkExtent2D.width;
        info.height = application->swapchain->swapchainVkExtent2D.height;
        info.layers = 1;
        for (uint32_t i = 0; i < imageCount; i++)
        {
