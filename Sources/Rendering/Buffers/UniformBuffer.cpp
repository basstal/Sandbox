...
// {
//     m_device = device;
//     mvpObjectBuffers.resize(Swapchain::MAX_FRAMES_IN_FLIGHT);
//     mvpObjectBuffersMapped.resize(Swapchain::MAX_FRAMES_IN_FLIGHT);
//     for (size_t i = 0; i < Swapchain::MAX_FRAMES_IN_FLIGHT; i++)
//     {
//         mvpObjectBuffers[i] = std::make_shared<Buffer>(device, sizeof(MVPObject), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
//                                                        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
//     }
//
//     // pbrMaterialBuffers.resize(Swapchain::MAX_FRAMES_IN_FLIGHT);
//     // pbrMaterialBuffersMapped.resize(Swapchain::MAX_FRAMES_IN_FLIGHT);
//     // for (size_t i = 0; i < Swapchain::MAX_FRAMES_IN_FLIGHT; i++)
//     // {
...
//     // }
//
//     pbrLightBuffers.resize(Swapchain::MAX_FRAMES_IN_FLIGHT);
//     pbrLightBuffersMapped.resize(Swapchain::MAX_FRAMES_IN_FLIGHT);
//     for (size_t i = 0; i < Swapchain::MAX_FRAMES_IN_FLIGHT; i++)
//     {
//         pbrLightBuffers[i] = std::make_shared<Buffer>(device, sizeof(Light), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
//                                                       VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
//     }
// }
//
...
// }
//
// MVPObject UniformBuffer::UpdateMVP(uint32_t currentImage, const std::shared_ptr<Camera>& camera, const glm::mat4& model,
//                                     const glm::mat4& projection)
// {
//     vkMapMemory(m_device->vkDevice, mvpObjectBuffers[currentImage]->vkDeviceMemory, 0, sizeof(MVPObject), 0, &mvpObjectBuffersMapped[currentImage]);
//     auto buffer = mvpObjectBuffers[currentImage];
//
//     MVPObject ubo;
//     ubo.model = model;
...
//     vkUnmapMemory(m_device->vkDevice, mvpObjectBuffers[currentImage]->vkDeviceMemory);
//     return ubo;
// }
//
// MVPObject UniformBuffer::UpdateMVP(uint32_t currentImage, const glm::mat4& view, const glm::mat4& model, const glm::mat4& projection)
// {
//     vkMapMemory(m_device->vkDevice, mvpObjectBuffers[currentImage]->vkDeviceMemory, 0, sizeof(MVPObject), 0, &mvpObjectBuffersMapped[currentImage]);
//     auto buffer = mvpObjectBuffers[currentImage];
//     MVPObject ubo;
...
// // }
//
// void UniformBuffer::UpdatePBRLight(uint32_t currentImage, const glm::vec3& position, const glm::vec3& color)
// {
//     vkMapMemory(m_device->vkDevice, pbrLightBuffers[currentImage]->vkDeviceMemory, 0, sizeof(Light), 0, &pbrLightBuffersMapped[currentImage]);
//     auto buffer = pbrLightBuffers[currentImage];
//     Light light(position, color);
//     // light.intensity = 1.0f;
//     memcpy(pbrLightBuffersMapped[currentImage], &light, sizeof(light));
//     vkUnmapMemory(m_device->vkDevice, pbrLightBuffers[currentImage]->vkDeviceMemory);
// }
//
// void UniformBuffer::UpdateWriteDescriptorSet(const std::shared_ptr<DescriptorResource>& descriptorResource)
// {
//     for (size_t i = 0; i < Swapchain::MAX_FRAMES_IN_FLIGHT; i++)
//     {
