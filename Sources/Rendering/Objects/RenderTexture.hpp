...
public:
	...
	VkSampler vkSampler;
	VkFormat vkFormat;
	RenderTexture(const std::shared_ptr<Device>& device, const std::shared_ptr<CommandResource>& commandResource, bool isCubeMap, bool isHdrImage, uint32_t width, uint32_t height, uint32_t mipLevels);
	~RenderTexture();
	void AssignImageData(const std::shared_ptr<Image>& image);
	void Cleanup();
