...
namespace Sandbox
{
    ...
    {
    public:
        DescriptorSet(const std::shared_ptr<Device>& device, const std::shared_ptr<DescriptorPool>& descriptorPool, const std::shared_ptr<DescriptorSetLayout>& descriptorSetLayout,
                      const BindingMap<VkDescriptorBufferInfo>& inBufferInfoMapping, const BindingMap<VkDescriptorImageInfo>& inImageInfoMapping);
        ...
        void Prepare(const std::map<uint32_t, std::vector<VkDescriptorBufferInfo>>& inBufferInfoMapping, const std::map<uint32_t, std::vector<VkDescriptorImageInfo>>& inImageInfoMapping,
