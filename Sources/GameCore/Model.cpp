...
void Model::PrepareRenderData(const std::shared_ptr<Device>& device, const std::shared_ptr<CommandResource>& commandResource)
{
    ...
    vertexBuffer = std::make_shared<Buffer>(device, bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    vertexBuffer->AssignData(Vertices().data());
    ...
    indexBuffer->AssignData(Indices().data());
}
